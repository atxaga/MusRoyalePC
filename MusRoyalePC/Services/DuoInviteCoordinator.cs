...
public sealed class DuoInviteCoordinator
{
    ...
    public async Task AcceptInviteAsync(DuoInvite invite)
    {
        try
        {
            ...
            // Suscripción única
            _service.OnStateChanged -= HandleState;
            _service.OnStateChanged += HandleState;
        ...
        {
            Application.Current.Dispatcher.BeginInvoke(() => MessageBox.Show(ex.Message));
        }

        void HandleState(DuoMatchState state)
        {
    ...
    private async Task StartDuoMatchAsync(string partidaId, int apuesta)
    {
        ...
        try
        {
            ...
            {
                Application.Current.Dispatcher.BeginInvoke(() => MessageBox.Show("Ezin izan da ID_ESKATU partidan sartu."));
            }
        }
        catch (Exception ex)
        {
